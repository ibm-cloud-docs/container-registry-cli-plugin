...
要了解有关必需许可权的信息，请参阅[用于使用 {{site.data.keyword.registrylong_notm}} 的访问角色](/docs/services/Registry?topic=registry-iam#access_roles_using)。

**命令选项**
<dl>
<dt>`DIRECTORY`</dt>
<dd>构建上下文的位置，包含 Dockerfile 和必备文件。如果在工作目录设置为构建上下文的存储位置时运行此命令，那么可以用句点 (.) 替换 `DIRECTORY`.</dd>
<dt>`--no-cache`</dt>
...
<dd>（可选）以“`KEY=VALUE`”格式指定其他构建自变量。通过包含此参数多次，可以指定多个构建自变量。指定与 Dockerfile 中的键相匹配的 ARG 行时，每个构建自变量的值都可用作环境变量。</dd>
...
构建不使用先前构建的构建高速缓存的 Docker 映像，其中构建输出已禁止，标记为 *`us.icr.io/birds/bluebird:1`*，并且该目录就是您的工作目录。
...
要了解有关必需许可权的信息，请参阅[用于配置 {{site.data.keyword.registrylong_notm}} 的访问角色](/docs/services/Registry?topic=registry-iam#access_roles_configure)。
...
<dd>要豁免的安全问题的标识。要查找问题标识，请运行 `ibmcloud cr va <image>`，其中 `<image>` 是映像的名称，并使用**漏洞标识**或**配置问题标识**列中的相关值。
</dd>
</dl>

**示例**

针对 `us.icr.io/birds/bluebird` 存储库中的所有映像，对标识为 `CVE-2018-17929` 的 CVE 创建 CVE 豁免。
...
针对标记为 `us.icr.io/birds/bluebird:1` 的单个映像的问题 `application_configuration:nginx.ssl_protocols`，创建配置问题豁免。
...
<dl>
<dt>`--scope SCOPE`</dt>
<dd>（可选）仅列出适用于此作用域的豁免。要将名称空间、存储库或标记设置为作用域，请输入下列其中一种格式的值：`namespace`、`namespace/repository` 或 `namespace/repository:tag`
</dd>
</dl>

**示例**

列出适用于 *`birds/bluebird`* 存储库中的映像的安全问题的所有豁免。输出包括帐户范围的豁免、作用域限定为 *`birds`* 名称空间的豁免，以及作用域限定为 *`birds/bluebird`* 存储库但不限定于 *`birds/bluebird`* 存储库中任何特定标记的豁免。
...
针对 `us.icr.io/birds/bluebird` 存储库中的所有映像，对标识为 `CVE-2018-17929` 的 CVE 删除 CVE 豁免。
...
针对标识为 `CVE-2018-17929` 的 CVE，删除帐户范围的 CVE 豁免。
...
针对标记为 `us.icr.io/birds/bluebird:1` 的单个映像的问题 `application_configuration:nginx.ssl_protocols`，删除配置问题豁免。
...
如果是使用 IAM 认证，那么此命令将启用细颗粒度授权。有关更多信息，请参阅[使用 Identity and Access Management 管理用户访问](/docs/services/Registry?topic=registry-iam#iam)和[定义用户访问角色策略](/docs/services/Registry?topic=registry-user#user)。
...
<dt>`--quiet`, `-q`</dt>
<dd>（可选）每个映像都以格式 `repository:tag` 列出。</dd>
...
向映像 `us.icr.io/birds/bluebird:1` 添加其他标记引用 `latest`。
...
将映像 `us.icr.io/birds/bluebird:peck` 复制到您有权访问的另一个名称空间 `animals`。
...
<dl>
<dt>`PLAN`</dt>
<dd>（可选）要升级到的价格套餐的名称。如果未指定 `PLAN`，缺省值为 `standard`。</dd>
</dl>
...
在 {{site.data.keyword.registrylong_notm}} 名称空间 *`birds`* 中导入 IBM 软件并将其打包以便与 Helm 配合使用，其中压缩文件的路径为 *`downloads/compressed_file.tgz`*。
...
<dd>（可选）将流量配额更改为指定的值（以兆字节为单位）。如果您无权设置流量，或者如果设置的值超过您的当前价格套餐，那么操作会失败。</dd>
<dt>`--storage STORAGE`</dt>
<dd>（可选）将存储配额更改为指定的值（以兆字节为单位）。如果您无权设置存储配额，或者如果设置的值超过您的当前价格套餐，那么操作会失败。</dd>
</dl>
...
添加不会到期且具有读/写访问权的令牌，并包含描述 *Token for my account*。
...
使用格式化伪指令 *`"{{ if eq .ReadOnly true}}{{.ID}} - {{.Expiry}} - {{.ReadOnly}} - {{.Description}}{{ end }}"`* 来显示所有只读令牌。
